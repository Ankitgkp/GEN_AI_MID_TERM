...
graph TD
    ...
    H --> I[Save model_pipeline.pkl]
    
    I --> J[Streamlit UI App]
    K[User Inputs via Web UI] --> J
    J --> L[Predict Churn Probability]
    L --> M[Display Insights & Risk]
```
...
Based on precision and F1 balance, the Logistic Regression pipeline was serialized (`model_pipeline.pkl`) and loaded into the Streamlit application for inference.

### 5. Running the Application
1. Ensure dependencies are installed (`pip install -r requirements.txt` or install `pandas scikit-learn matplotlib seaborn streamlit`).
2. Train the pipeline (automatically triggered, or run `churn.ipynb`).
